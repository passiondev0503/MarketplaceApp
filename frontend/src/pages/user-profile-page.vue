        ...
        <div class="user-info-container">
          <div>
            <div class="my-orders-table">
              <el-table
                v-if="orders && incommingOrders && inbox"
                :data="ordersToShow"
                stripe
                style="width: 100%"
              >
          ...
          <div class="reviews-table">
            <el-table
              v-if="reviews && userStays"
              :data="reviewsToShow"
              stripe
              style="width: 100%"
            >
              <el-table-column prop="name" label="Stay name"> </el-table-column>
              <el-table-column
                prop="review.by.fullname"
                label="User"
                width="120px"
              >
              </el-table-column>
  ...
  computed: {
    ...
    ordersToShow() {
      this.incommingOrders = this.incommingOrders.map((order) => {
        console.log(order);
        order.totalPrice = order.totalPrice.toLocaleString("en-US", {
          currency: "USD",
          style: "currency",
          maximumFractionDigits: 0,
        });
        ...
        order.Dates =
          order.Dates[0].toString().slice(3, 10) +
          " - " +
          order.Dates[1].toString().slice(3, 15);
    ...
    myOrdersToShow() {
      this.userOrders = this.userOrders.map((order) => {
        ...
        order.Dates =
          order.Dates[0].slice(0, 10) + " - " + order.Dates[1].slice(0, 10);
    ...
    staysToShow() {
      this.userStays = this.userStays.map((stay) => {
        stay.price = stay.price.toLocaleString("en-US", {
          currency: "USD",
          style: "currency",
          maximumFractionDigits: 0,
        });
